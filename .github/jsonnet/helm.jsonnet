{
  deployHelm(cluster, release, values, chartPath, delete=false, useHelm3=false, title=null, ifClause=null)::
    $.action((if title == null then if delete then 'delete-helm' else 'deploy-helm' else title),
      $.helm_action_image,
      with = {
  ...
  helmDeletePRJob(serviceName,
    options = {},
    helmPath = './helm/' + serviceName,
    deploymentName = serviceName + '-pr-${{ github.event.number }}',
    mysqlDeleteOptions = { enabled: false },
  )::
    $.ghJob('helm-delete-pr',
      image=$.default_job_image,
      useCredentials=false,
      steps=[
        $.checkout(),
  ...
  helmDeletePRPipeline(serviceName,
    options = {},
    helmPath = './helm/' + serviceName,
    deploymentName = serviceName + '-pr-${{ github.event.number }}',
  )::
  ...
  helmDeployCanaryJob(serviceName,
    options = {},
    helmPath = './helm/' + serviceName + '-canary',
    deploymentName = serviceName + '-canary',
    image = $.default_job_image,
